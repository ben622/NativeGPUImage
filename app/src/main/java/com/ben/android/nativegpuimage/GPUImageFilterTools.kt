...
object GPUImageFilterTools {
    fun showDialog(
            context: Context,
            listener: (filter: NativeFilter) -> Unit
    ) {
    ...
    private fun createFilterForType(context: Context, type: FilterType): NativeFilter {
        return when (type) {
    ...
    class FilterAdjuster(filter: NativeFilter) {
        private val adjuster: Adjuster<out NativeFilter>?
        ...
        fun getFilter():NativeFilter?{
            if(adjuster==null) return null
            return adjuster!!.getAdjusterFilter()
        }

        private abstract inner class Adjuster<T : NativeFilter>(protected val filter: T) {
            ...
            fun getAdjusterFilter(): NativeFilter {
                return filter
            }
        }
        ...
        private inner class SobelAdjuster(filter: GPUImageSobelEdgeDetectionFilter) :
            ...
            override fun adjust(percentage: Int) {
                filter.setSize(range(percentage, 0.0f, 5.0f))
            }
        }
